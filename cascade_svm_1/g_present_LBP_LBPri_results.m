...
p(i) = plot(X_LR,Y_LR,'g', 'LineWidth', 2);
legend_text{i} = ['Heart ( AUC = ' num2str(AUC_LR) ' )'];

i = i + 1;
[X_L,Y_L,T_L,AUC_L] = perfcurve(testDatay, prob_estimates_LR(:,3),svmmodel_LR.Label(3)); 
p(i) = plot(X_L,Y_L,'m', 'LineWidth', 2);
legend_text{i} = ['Muscular ( AUC = ' num2str(AUC_L) ' )'];

i = i + 1;
[X_R,Y_R,T_R,AUC_R] = perfcurve(testDatay, prob_estimates_LR(:,4),svmmodel_LR.Label(4));
p(i) = plot(X_R,Y_R,'b', 'LineWidth', 2);
legend_text{i} = ['Elastic-Vein ( AUC = ' num2str(AUC_R) ' )'];
